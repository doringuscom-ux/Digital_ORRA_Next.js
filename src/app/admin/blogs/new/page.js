...
export default function CreateNewBlogPage() {
  ...
  // Handle Title input with Auto-Slug
  ...
  // Upload In-Article Image to Cloudinary and Insert into Content visually
  const handleInlineImageUpload = async (e) => {
    ...
    try {
      ...
      if (res.ok && data.url) {
        ...
        setFeedback({ type: "success", text: "Image inserted into article visually!" });
      } else {
  ...
  const handlePublish = async (e) => {
    ...
    try {
      const res = await fetch("/api/blogs", {
        ...
        body: JSON.stringify({ ...formData, content: finalContent }),
      });

      const data = await res.json();
      if (res.ok) {
        setFeedback({ type: "success", text: "Blog published successfully! Redirecting..." });
        setTimeout(() => {
          router.push("/admin/blogs");
        }, 1200);
      } else {
        setFeedback({ type: "error", text: data.message || "Failed to publish." });
      }
    } catch (err) {
      setFeedback({ type: "error", text: err.message || "Server error while saving." });
    } finally {
      setSaving(false);
    }
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-white/10">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl sm:text-2xl font-black text-white flex items-center gap-2">
              Create New <span className="text-gradient">Blog Article</span>
            </h1>
            <p className="text-xs text-gray-400">
              Compose rich content with live real-time preview on the right.
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            disabled={saving}
            className="flex items-center gap-2 px-6 py-3 rounded-2xl font-bold text-xs sm:text-sm text-white bg-gradient-to-r from-[#FF3399] to-[#d6006e] hover:from-[#ff4d9d] hover:to-[#e60077] shadow-[0_8px_20px_rgba(255,51,153,0.35)] transition-all cursor-pointer disabled:opacity-50"
          >
            {saving ? (
              <>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-8 space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. SEO, Social Media, Growth"
          ...
          <div className="rounded-3xl bg-[#0F1B4C]/40 border border-white/10 p-6 backdrop-blur-xl shadow-xl space-y-4">
            ...
            {/* Hidden Input for In-Article Inline Image Upload */}
            <input
              type="file"
            ...
            <div className="flex flex-wrap items-center gap-2 p-2.5 bg-[#0A1128] border border-white/10 rounded-2xl">
              {/* Heading Dropdown */}
              <div className="relative">
                <select
                  defaultValue="<p>"
                ...
                <div className="absolute right-2.5 top-1/2 -translate-y-1/2 pointer-events-none text-cyan-400 text-[10px]">▼</div>
              </div>
              ...
              <button
                ...
                className="px-2.5 py-1.5 rounded-lg bg-[#FF3399]/10 hover:bg-[#FF3399]/20 text-[#FF3399] border border-[#FF3399]/30 text-xs font-bold flex items-center gap-1.5 cursor-pointer disabled:opacity-50"
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          {/* Card 4: SEO Metadata & Google SERP Snippet */}
          <div className="rounded-3xl bg-[#0F1B4C]/50 border border-white/10 p-5 sm:p-6 backdrop-blur-xl shadow-xl space-y-4">
            <div className="flex items-center justify-between pb-2 border-b border-white/10">
